...
class ReleaseArch < ApplicationRecord
  ...
  scope :search_id, -> (term) {
    ...
    where('release_arches.id::text ILIKE ?', "%#{sanitize_sql_like(identifier)}%")
  }
  ...
  scope :search_key, -> (term) {
    ...
    where('release_arches.key ILIKE ?', "%#{sanitize_sql_like(term)}%")
  }
  ...
  scope :search_name, -> (term) {
    ...
    where('release_arches.name ILIKE ?', "%#{sanitize_sql_like(term)}%")
  }
